...
export default function Footer() {
  return (
    <footer className="bg-brown-900 border-t border-brown-700/50">
      <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8">
        <div className="grid gap-12 lg:grid-cols-4">
          ...
          {/* Link columns */}
          <div>
            <h4 className="font-display text-xs font-semibold tracking-[0.15em] text-cream/80 uppercase">
              Retreats
            </h4>
            <ul className="mt-4 space-y-3">
              {footerLinks.retreats.map((link) => (
                <li key={link.label}>
                  <a
                    href={link.href}
...
>
        ...
        {/* Bottom bar */}
        <div className="mt-16 flex flex-col items-center justify-between gap-4 border-t border-brown-700/50 pt-8 sm:flex-row">
          <p className="text-xs text-cream/30">
            &copy; {new Date().getFullYear()} Mai Niti Alternative. All rights
            reserved.
          </p>
          <p className="text-xs text-cream/30">
            Plant medicine healing retreats in Pucallpa, Peru
          </p>
        </div>
      </div>
    </footer>
